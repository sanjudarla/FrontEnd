...
const Doctor = () => {
    ...
    const getData = () => {
        axios.get('https://localhost:44327/api/Doctor')
            .then((result) => {
                setData(result.data)
            })
            .catch((error) => {
    ...
    const handleEdit = (id) => {
        ...
        handleShow();
        axios.get(`https://localhost:44327/api/Doctor/${id}`)
            .then((result) => {
    ...
    const handleDelete = (id) => {
        if (window.confirm("Are you sure want to delete this record") === true) {
            axios.delete(`https://localhost:44327/api/Doctor/${id}`)
                .then((result) => {
                    clear();
    ...
    const handleUpdate = () => {
        const url = `https://localhost:44327/api/Doctor/${EditDoctor_Id}`;
        const data = {
    ...
    const handleSave = () => {
        const url = 'https://localhost:44327/api/Doctor/';
        const data = {
            "Doctor_Id": Doctor_Id,
